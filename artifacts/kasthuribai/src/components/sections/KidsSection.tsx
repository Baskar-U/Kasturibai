...
import { PRODUCTS, Product, Color, Subcategory, Style, AgeRange } from "@/data/mock-data";
...
const KIDS_COLORS: Color[] = ["Red", "Blue", "Yellow", "Green", "Pink", "Purple", "White"];
const KIDS_SIZES: string[] = ["2-3Y", "4-5Y", "6-7Y", "8-9Y", "10-11Y", "12Y"];
const KIDS_AGE_RANGES: AgeRange[] = ["1-3 yrs", "4-8 yrs", "9-12 yrs"];
const KIDS_STYLES: Style[] = ["Casual Wear", "Daily Wear", "Party Wear", "Traditional", "Festive"];
...
export function KidsSection({ onViewProduct }: KidsSectionProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeTab === "new") {
      ...
      result = result.filter((p) => p.styles?.includes("Party Wear"));
    } else if (activeTab === "cute") {
      result = result.filter((p) => p.badges?.includes("Hot") || p.badges?.includes("Trending"));
    }
    ...
    return result;
  }, [kidsProducts, filters, activeTab]);

  const tabs = [
    { id: "all", label: "All", icon: "🧸" },
    { id: "new", label: "New Arrivals", icon: "🆕" },
    { id: "party", label: "Party Wear", icon: "🎉" },
    { id: "cute", label: "Cute Picks", icon: "💖" },
  ];

  return (
    <section id="kids" className="py-12 sm:py-16 md:py-24 bg-gradient-to-b from-green-50/50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8 sm:mb-10">
          ...
          <p className="text-sm sm:text-base text-muted-foreground font-body mt-4 max-w-2xl mx-auto">
            Colorful & Comfortable Collection for Kids
          </p>
        </div>
        ...
        <div className="flex gap-6 lg:gap-8">
          ...
          <FilterPanel
            ...
            availableSubcategories={KIDS_SUBCATEGORIES}
            availableColors={KIDS_COLORS}
            availableSizes={KIDS_SIZES}
            availableAgeRanges={KIDS_AGE_RANGES}
            availableStyles={KIDS_STYLES}
            showAgeFilter={true}
